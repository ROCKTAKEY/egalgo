...
#+BEGIN_SRC emacs-lisp -n
  ...
  ;; Minimum running.
  (egalgo-run
   '(t t t t t t t t t t)                 ;t means bool nucleotide.
   #'knapsack-rater)
  ...
  (egalgo-run
   ...
   :mutation 0.05       ;Probability of each nucleotide mutating.
  ...
  (egalgo-run
   ...
   :size 30
   :mutation 0.01
   :n-point-crossover t)                  ;unicrossover
#+END_SRC
...
** ~selector~ (optional, keyword)
   ...
   - Should take 1 argument, which is list of rate of each chromosome
   - Should return index of selected chromosome
   - If rate is ~nil~, the chromosome must not be selected.

   This can be alias, which is defined in ~egalgo-selector-alias~.

** ~termination~ (optional, keyword)
   The number of maximum generation. Finish algorithm when generation
   become the value.

   This should be positive integer.
   In the future, this will be able to function which returns
